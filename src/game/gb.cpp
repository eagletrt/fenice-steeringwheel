...
#define SCANCODE_DOWN 0
#define SCANCODE_UP 1
#define SCANCODE_LEFT 2
#define SCANCODE_RIGHT 3
#define SCANCODE_TAB 4
#define SCANCODE_RETURN 5
#define SCANCODE_Z 6
#define SCANCODE_X 7
...
#define OP9_IMM_PTR(_) OP8_REL(_) case _ + 70 : operand = opcode & 64 ? mem8(PC++) : tmp8;

uint8_t opcode, opcrel, tmp8, operand, carry, neg, *rom0, *rom1, io[512], video_ram[8192], work_ram[16384], *extram,
    *extrambank, reg8[] = {19, 0, 216, 0, 77, 1, 176, 1}, &F = reg8[6], &A = reg8[7],
                 *reg8_group[] = {reg8 + 1, reg8, reg8 + 3, reg8 + 2, reg8 + 5, reg8 + 4, &F, &A}, &IF = io[271],
                 &LCDC = io[320], &LY = io[324], IME, halt;

uint8_t const key_state[] = {0, 0, 0, 0, 0, 0, 0, 0};

uint16_t PC = 256, *reg16 = (uint16_t *)reg8, &HL = reg16[2], SP = 65534, &DIV = (uint16_t &)io[259], ppu_dot = 32,
...
uint8_t GB::mem8(uint16_t addr, uint8_t val, int write) {
  tick();
  switch (addr >> 13) {
  case 1:
    if (write)
  ...
  case 7:
    if (addr >= 65024) {
      ...
      if (addr == 65280) {
        if (~io[256] & 16)
          return ~(16 + key_state[SCANCODE_DOWN] * 8 + key_state[SCANCODE_UP] * 4 + key_state[SCANCODE_LEFT] * 2 +
                   key_state[SCANCODE_RIGHT]);
        if (~io[256] & 32)
          return ~(32 + key_state[SCANCODE_RETURN] * 8 + key_state[SCANCODE_TAB] * 4 + key_state[SCANCODE_Z] * 2 +
                   key_state[SCANCODE_X]);
        return 255;
      }
      return io[addr & 511];
    }
...
void GB::set_flags(uint8_t mask, int Z, int N, int H, int C) { F = F & mask | !Z << 7 | N << 6 | H << 5 | C << 4; }
...
void GB::push(uint16_t val) {
  mem8(--SP, val >> 8, 1);
  mem8(--SP, val, 1);
  tick();
}

uint8_t GB::reg8_access(uint8_t val, int write, uint8_t o) {
  return (o &= 7) == 6 ? mem8(HL, val, write) : write ? *reg8_group[o] = val : *reg8_group[o];
}

uint8_t GB::get_color(int tile, int y_offset, int x_offset) {
  ...
  while (1) {
    ...
    if (IME & IF & io[511]) {
      ...
      SUBTRACT:
        ...
        operand = ~operand;
      ALU:
        set_flags(0, tmp8 = A + operand + carry, neg, (A % 16 + operand % 16 + carry > 15) ^ neg,
      ...
      case 250:
        ...
        tmp = opcode & 16;
        reg8_access(mem8(opcode & 8 ? read16() : 65280 + (opcode & 2 ? *reg8 : mem8(PC++)), A, !tmp), tmp, 7);
        break;
              ...
              if (LCDC & 2)
                for (uint8_t *sprite = io; sprite < io + 160; sprite += 4) {
                  uint8_t sprite_x = tmp - sprite[1] + 8, sprite_y = LY - *sprite + 16,
                          sprite_color = get_color(sprite[2], sprite_y ^ (sprite[3] & 64 ? 7 : 0),
