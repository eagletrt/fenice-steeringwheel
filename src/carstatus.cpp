...
// Set Brake value
void CarStatus::setBrake(int val) { sensors.setBrakeVal((int)val); }
...
void CarStatus::setThrottle(int val) {
  ...
  // race.setVelocity(((80.0 * currPercMap * m_throttleVal) / 10000.0));
  // //remember to uncomment the velocityChanged emit on processingTimeout in
  // order to emit the velocity
}
...
bool CarStatus::getSender() { return telemetry.getSender(); }
...
void CarStatus::setAPPSBSEStatus(int apps, int bse) {
...
int CarStatus::carStatus() { return control.getCarStatus(); }

int CarStatus::getTc() { return manettini.getTc(); }

int CarStatus::getPump() { return manettini.getPump(); }

int CarStatus::getMap() { return manettini.getMap(); }
...
int CarStatus::pump() const { return manettini.getPump(); }
int CarStatus::map() const { return manettini.getMap(); }
int CarStatus::tc() const { return manettini.getTc(); }
int CarStatus::velocity() const { return race.getVelocity(); }
int CarStatus::brakeVal() const { return sensors.getBrakeVal(); }
...
int CarStatus::lvVolt() const { return lv.getLvVolt(); }
int CarStatus::getCtrlIsOn() { return this->control.getCtrlIsOn(); }
