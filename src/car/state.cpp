...
void State::handleMessage(const CanDevice *, quint32 id, const QByteArray &message) {
  ...
  if (id == ID_INVERTER_LEFT_TEMPERATURE_IGBT) {
    Primary_INVERTER_LEFT_TEMPERATURE_IGBT data;
    deserialize_Primary_INVERTER_LEFT_TEMPERATURE_IGBT(raw, &data);
    m_inverters->setLeftTemperature(floor((data.value - 15797) / 112.1182));
    emit invertersChanged();
  } else if (id == ID_INVERTER_LEFT_TEMPERATURE_IGBT) {
    Primary_INVERTER_RIGHT_TEMPERATURE_IGBT data;
    deserialize_Primary_INVERTER_RIGHT_TEMPERATURE_IGBT(raw, &data);
    m_inverters->setRightTemperature(floor((data.value - 15797) / 112.1182));
    emit invertersChanged();
  } else if (id == ID_BMS_HV_VOLTAGE) {
    Secondary_BMS_HV_VOLTAGE data;
    deserialize_Secondary_BMS_HV_VOLTAGE(raw, &data);
    m_hv->setPackVoltage(floor(data.total / 10000.));
    emit hvChanged();
  } else if (id == ID_PEDALS_THROTTLE) {
    Primary_PEDALS_THROTTLE data;
    deserialize_Primary_PEDALS_THROTTLE(raw, &data);
    m_hv->setPower(data.value * .8);
    emit hvChanged();
  } else if (id == ID_FRONT_WHEELS_ENCODERS_SPEED_RADS) {
    Primary_FRONT_WHEELS_ENCODERS_SPEED_RADS data;
    deserialize_Primary_FRONT_WHEELS_ENCODERS_SPEED_RADS(raw, &data);
    m_ecu->setLeftSpeedRads(floor(data.left / 1000.));
    m_ecu->setRightSpeedRads(floor(data.right / 1000.));
    m_ecu->setSpeed(abs(floor(((data.left / 1000.) * 0.2 * 3.6 * 120) / 1000)));
  }
