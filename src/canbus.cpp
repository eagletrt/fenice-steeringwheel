...
Canbus::Canbus(CarStatus *m_carStatus) {
  ...
  connect(timerSteeringWheel, SIGNAL(timeout()), this, SLOT(steerConnected()));

  connect(timerEnc, SIGNAL(timeout()), this, SLOT(sendEncState()));

  connect(timerStatus, SIGNAL(timeout()), this, SLOT(askStatus()));

  connect(timerTelemetry, SIGNAL(timeout()), this, SLOT(sendTelemetry()));

  // connect(device, SIGNAL(framesReceived()),
  // this, SLOT(parseSerial()));

  connect(carStatus, SIGNAL(toggleCar()), this, SLOT(toggleCar()));

  connect(carStatus, SIGNAL(CTRLEnabledChanged()), this, SLOT(toggleCar()));
...
void Canbus::sendTelemetry() {
  if (carStatus->getSender()) {
    ...
    // qDebug() << "telemetry[1] = " << telemetry[1] << "telemetry[2] = " <<
    // telemetry[2];
...
void Canbus::sendCanMessage(int id, QByteArray message) {
  ...
  if (message[0] == char(101)) {
    qDebug() << frame.toString();
  }
  if (m_hasCan)
    device->writeFrame(frame);
  else
    qDebug() << "Not can interface. Tried to send " << id;
}
...
void Canbus::parseCANMessage(int mid, QByteArray msg) {
  ...
  case ECU_MSG:
    ...
    } else if (msg.at(0) == 0x04) // idle
    {
      ...
      carStatus->setCarStatus(ctrlIsEnabled, ctrlIsOn, driveModeEnabled, 0, 0);
  ...
  case LV_ID:
    ...
    carStatus->setLVStatus((uint8_t)msg.at(0), (uint8_t)msg.at(2));
